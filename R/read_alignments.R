...
#' @export
#' @examples
#' read_alignments(c('alnA.fa','alnB.fa'), nReads = 100, naming = 'filenames')
...
read_alignments = function(input, nReads = -1L, naming = 'headers') {
      ...
      sequences = unlist(lapply(1:length(input), function(x) {
	    ...
	    seqs = dat[c(F,T)] 
	    if(naming == 'headers') return(setNames(seqs, gsub('>', '', dat[c(T,F)], fixed = T)))
	    rm(dat)
	    if(naming == 'filenames') return(setNames(seqs, rep(gsub('.nt.fasta$|.aa.fasta$|.fasta$|.fa$','',basename(input[x])), length(seqs))))
      }))
      return(sequences)
}
